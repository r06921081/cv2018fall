...
from dataloader import cv_loader as DA
import cv2
from models import *
import sys
...
elif args.datatype == 'cv':
    from dataloader import cv_synthesis as ls
elif args.datatype == 'real':
    from dataloader import cv_real as ls
...
def test(imgL,imgR,disp_true):
        ...
        result = pred_disp.data.numpy()
        #print(result.shape)
        result = np.reshape(result, (368, 1232))
        #print(result)
        #print(disp_true)
        #writePFM(str(count)+"result.pfm", cv2.resize(result, (384,512)).astype(np.float32))
        cv2.imwrite(str(int(count%10))+"result.png", (result).astype(np.uint8))
        count += 1
