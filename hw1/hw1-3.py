...
class bilateral_filter(object):
  def __init__(self, sigma_s, sigma_r, spatial_kernal, weight = (0.114, 0.587, 0.299), file = None):
    if file is not None:
      self.img = file
    else:
  ...
  def filte(self):
    for i in range(self.l):
      for j in range(self.w):
        self.BGRjbfdst[i, j] += np.sum(self.BGRborded[i:i+2*self.r+1, j:j+2*self.r+1] * self.filters[i*self.w + j], axis=(0, 1))
        pass
  ...
  def BGRjointbilateralfilter(self):
    ...
    for i in range(r, r + self.l):
      for j in range(r, r + self.w):
        ...
        h_r += np.repeat(np.reshape(self.gborded[i-r:i+r+1, j-r:j+r+1] - self.gborded[i, j], (self.kernel_size, self.kernel_size, 1)), 3, axis=2)      

        
        h_r = np.exp(-(h_r**2 / (2 * (self.sigma_r**2)))) #* self.f

        h_r = np.multiply(self.f, h_r) 
        h_r = h_r / np.sum(h_r, axis=(0, 1))
        self.filters.append(h_r)
        # self.filters.append(self.BGRjointconv(i, j))
        pass
    self.filte()  
    print('use', now() - begin, 'sec')
...
if __name__ == '__main__':
  ...
  # 66 combination vote vector
  vote = [0] * 66

  
  begin = now()
  # 9 combination of sigma_s and sigma_r and the spactial_kerner dict
  sigmas = []
  spactial_kerner = {}
  for sigma_s in [1, 2, 3]:
    for sigma_r in [0.05, 0.1, 0.2]:
      ...
      r = sigma_s * 3
      s_f = np.zeros((2*r + 1, 2*r + 1, 3))
      for i in range(-r, r + 1):
        for j in range(-r, r + 1):
          value = np.exp(-(i**2 + j**2) / (2 * sigma_s**2))
          s_f[i + r, j + r] = [value, value, value]
          # self.f[i + r, j + r] += [np.exp(-(i**2 + j**2) / (2 * self.sigma_s**2)), np.exp(-(i**2 + j**2) / (2 * self.sigma_s**2)), np.exp(-(i**2 + j**2) / (2 * self.sigma_s**2))]
      spactial_kerner[(sigma_s, sigma_r)] = s_f
  print(sigmas)
  
  def sigpr(sigma_s, sigma_r):
    vo = [0] * 66
    inputimg = cv2.imread(sys.argv[1])
    raw = bilateral_filter(sigma_s, sigma_r, spactial_kerner[(sigma_s, sigma_r)], file=inputimg)
    bf = raw.BGRbilateralfilter()
    for weight in weights.keys():
      # if len(ths) == 6:
      #   while len(ths) != 0:
      #     ths.pop().join()
      print(weight)
      img = bilateral_filter(sigma_s, sigma_r, spactial_kerner[(sigma_s, sigma_r)], weight, inputimg)
      # img.gshow('src')
      # bf = img.BGRbilateralfilter()
      # img.BGRshow('dst')
      jbf = img.BGRjointbilateralfilter()
      ...
      # img.BGRjbfshow('dst')
      # cv2.imshow('d', bf - jbf)
      # cv2.imwrite('./jbf/' + str(weight) + '-' + str(sigma_s) + ',' + str(sigma_r) + '.png', (jbf)*255)
      # cv2.imwrite('./d/' + str(weight) + '-' + str(sigma_s) + ',' + str(sigma_r) + '.png', (bf - jbf)*255*10)
      # cv2.imwrite('./g/' + str(weight) + '.png', g*255)
  ...
  prs = []
  # 9 combination of sigma to do multiprocess
  for ss in sigmas:
    # sigpr(ss[0], ss[1], bf)
    # continue
    pr = Process(target=sigpr, args=(ss[0], ss[1], ))
    pr.start()
    prs.append(pr)
    # break
  ...
  for contest in re:
    ...
    for weight in weights:
      ...
      # if the center point of neighbor points is smallest
      if mincount == len(neibs):
        # leaderboard[x] = [diff_value, index_of_vote, neibs_number]
        leaderboard.append([contest[weights[weight] - 1], weights[weight] - 1, len(neibs)])

    top = np.argsort(np.array(leaderboard)[:, 0], axis=0)
    # print(top)
    # weighted vote smallest one got 3 point ,biggest of top3 got 1 point
    for rank in top:
      vote[leaderboard[rank][1]] += (4 - rank) * int(leaderboard[rank][2]/2)
      # print(3 - rank)

  print(vote)
  print(np.argsort(vote))
  # print(vote[np.argsort(vote)[63]] , vote[np.argsort(vote)[62]])
  if vote[np.argsort(vote)[63]] == vote[np.argsort(vote)[62]]:
    print(np.argsort(vote)[65], np.argsort(vote)[64])
    re = [list(weights.items())[np.argsort(vote)[65]][0], list(weights.items())[np.argsort(vote)[64]][0]]
    print(re)
  else:
    print(np.argsort(vote)[65], np.argsort(vote)[64], np.argsort(vote)[63])
    re = list(weights.items())[np.argsort(vote)[65]][0], list(weights.items())[np.argsort(vote)[64]][0], list(weights.items())[np.argsort(vote)[63]][0]
    print(re)

  img = cv2.imread(sys.argv[1])
  for i, g in enumerate(re):
    raw = bilateral_filter(sigma_s, sigma_r, spactial_kerner[(sigma_s, sigma_r)], file=img, weight=g)
    result = raw.gray
    cv2.imwrite(sys.argv[2] + '/' + sys.argv[1].split('/')[-1].split('.')[-2] + '_y' + str(i+1) + '.' + sys.argv[1].split('.')[-1], result * 255)
  exit()
